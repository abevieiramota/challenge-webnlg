...
REFERENCE_BASEPATH = os.path.join(CURRENT_DIR, 'data-challenge')
REFERENCE_FILENAME = 'testdata_with_lex.xml'

TEAMS_FILENAME = {'ADAPT_Centre': './submissions/adaptcentre.txt',
                  'GKB_Unimelb': './submissions/melbourne.txt',
                  'PKUWriter': './submissions/pkuwriter.txt',
                  'Tilburg_University-1': './submissions/tilburg-nmt.txt',
                  'Tilburg_University-2': './submissions/tilburg-smt.txt',
                  'Tilburg_University-3': './submissions/tilburg-template.txt',
                  'UIT-DANGNT-CLNLP': './submissions/uit-vietnam.txt',
                  'UPF-TALN': './submissions/upf-forge.txt',
                  'Baseline': './submissions/baseline_sorted.txt'
                  }
...
def generate_reference_files():
    ...
    for cat in categories:
        b = Benchmark()
        b.fill_benchmark([(REFERENCE_BASEPATH, REFERENCE_FILENAME)])
    ...
    bk = Benchmark()
    bk.fill_benchmark([(REFERENCE_BASEPATH, REFERENCE_FILENAME)])
...
def read_participant(output_file, teamname, outdir):
    ...
    for size in range(1, 8):
        ...
        write_to_file(output_reduced, str(size)+'size', teamname, outdir)
    ...
    for category in categories:
        ...
        for entry in b.entries:
            if entry.category == category:
                entry_ids += [int(entry.id[2:])]      # entry id -- 'Id1'
        output_reduced = [output[i-1] for i in sorted(entry_ids)]
        write_to_file(output_reduced, category, teamname, outdir)
    ...
    for category in old_categories:
        ...
        for entry in b.entries:
            if entry.category == category:
                entry_ids += [int(entry.id[2:])]      # entry id -- 'Id1'
    output_reduced = [output[i-1] for i in sorted(entry_ids)]
    write_to_file(output_reduced, 'old-cat', teamname, outdir)
    ...
    for category in new_categories:
        ...
        for entry in b.entries:
            if entry.category == category:
                entry_ids += [int(entry.id[2:])]      # entry id -- 'Id1'
    output_reduced = [output[i-1] for i in sorted(entry_ids)]
    write_to_file(output_reduced, 'new-cat', teamname, outdir)
    
    # create all-category files
    write_to_file(output, 'all-cat', teamname, outdir)
    print('Files creating finished for: ', teamname)


def write_to_file(output_reduced, param, teamname, outdir):
    out = ''
    out_ter = ''
...
def generate_team_files(team=None, filepath=None, name=None, outdir=None):
    
    if team:
        
        filepath = TEAMS_FILENAME[team]
        name = team
    
    # generate teams
    read_participant(filepath, name, outdir)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--team_filepath')
    parser.add_argument('--outdir')
    
    args = parser.parse_args()
    
    # TODO: add keyword argument
    
    if args.team == 'all':
        
        for team in TEAMS_FILENAME.keys():
            
            generate_team_files(team=team, outdir=args.outdir)
            
    else:
        
        generate_team_files(filepath=args.team_filepath, 
                  name=args.team_name,
                  outdir=args.outdir)